...
## 2025-02-28 - [Privilege Escalation on User Auto-Creation]
**Vulnerability:** Any LDAP user logging into the system for the first time was automatically granted the `admin` role in the database.
**Learning:** The helper function `ensureUser` was hardcoding `Role: queries.UserRoleAdmin` in the `CreateUserParams` struct. Because this function was reused by both the `loginAsAdmin` and `loginAsLDAP` flows, LDAP users received administrative privileges unintentionally. Additionally, the role was hardcoded as a string `"user"` or `"admin"` in the JSON JWT response rather than taking the true database value.
**Prevention:** Helper functions creating users MUST accept a role parameter explicitly rather than hardcoding defaults. Token generation and authentication response DTOs MUST dynamically use the user's actual database role (e.g., `string(user.Role)`) to prevent desynchronization between tokens and actual database privileges.
