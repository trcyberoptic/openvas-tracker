...
- **Ticket dedup key is `(host, oid)` — `vulnerabilities.oid` is the OpenVAS NVT OID / ZAP `pluginid` (migration 023).** It is the only part of a finding that survives both a VT rename and a change in CVE references. `(host, title)` OR `(host, CVE)` remains as a fallback, and only matches rows whose `oid` is NULL — a *different* OID means a different check and must not merge. The fallback self-heals: `TouchTicket` repoints `vulnerability_id` at the new row, so legacy tickets carry an OID after one scan cycle. Why: Greenbone removed the DISPUTED CVE refs from 41 NVTs in the 2026-08-08 feed, the old "CVE else title" fingerprint flipped, and every affected host silently got a **second** ticket — resurrecting 17 false positives and 31 risk-accepts (repaired in prod, 136 merged). `FindTicketByFingerprint` (misleading name) matches only those forms; CWE/URL/parameter are NOT part of ticket identity. `AlsoAffectedHosts` deliberately mirrors the same identity notion — keep them in sync. All ZAP instances of the same alert on one host collapse into ONE ticket regardless of URL/param; affected URL+param pairs show in ticket detail via `AffectedURLsByPeer`. `TouchTicket`/`ReopenTicket` repoint `vulnerability_id` to the newest instance, so the detail page's URL/parameter fields reflect the most recent one only.
...
- An import that creates ≥50 new tickets logs a WARNING (`newTicketBurstThreshold` in `import.go`) — a burst means finding identity moved, not that the estate regressed. It is only a log line; nothing alerts.
- Flow per vuln: find ticket by (host+OID, else host+title OR host+CVE) → create/reopen/touch; risk rules are checked **only inside createTicket** (reopens bypass them) → auto-resolve stale → reopen expired risk-accepts → commit.
...
- **ticketCols / qualifiedTicketCols / scanTicket** — must stay in sync when adding ticket columns (`internal/database/queries/tickets.go`). Ticket queries JOIN vulnerabilities AND scans. Same for **vulnCols / scanVuln** (incl. `url`, `parameter`, `evidence`, `confidence`, `oid`).
...
- **Deleting scans cascades** — `DELETE FROM scans` removes the scan's `vulnerabilities` (CASCADE) and NULLs `tickets.vulnerability_id` (SET NULL), leaving the tickets alive but detached from their finding. Ticket queries JOIN vulnerabilities, so orphans drop out of most views without being deleted — prod carried 9 such tickets on 2026-08-10. Check with `SELECT COUNT(*) FROM tickets WHERE vulnerability_id IS NULL` before/after.
